    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.FinancialOrders") }}</h5>
          <button
            ...
            <template v-slot:default>
              ...
              <tbody>
                <tr v-for="row in tableRows" :key="row.id">
                  ...
                  <td>{{ row.user_mobile ? row.user_mobile : "-" }}</td>
                  <td>
                    {{
                      row.total_profit_with_tax
                        ? row.total_profit_with_tax
                        : "-"
                    }}
                  </td>
                  <td>{{ row.tax ? row.tax : "-" }}</td>
                  <td>{{ row.platform_profit ? row.platform_profit : "-" }}</td>
                </tr>
              </tbody>
            </template>
          </v-simple-table>
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.client-phone"),
          value: "user_name",
          sortable: false,
          align: "center",
        },
        {
          text: this.$t("PLACEHOLDERS.totalOrdersCount"),
          value: "user_mobile",
          sortable: false,
          align: "center",
        },
        {
          text: this.$t("PLACEHOLDERS.totalOrdersValue"),
          value: "total_profit_with_tax",
          sortable: false,
          align: "center",
        },
        {
          text: this.$t("PLACEHOLDERS.totalOrdersValueNoTax"),
          value: "tax",
          sortable: false,
          align: "center",
        },
        {
          text: this.$t("PLACEHOLDERS.totalTaxValue"),
          value: "platform_profit",
          sortable: false,
          align: "center",
        },
        // {
  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        this.users = res.data.data?.map((ele)=>({id : ele.id , name : ele.mobile , value : ele.mobile}));
      } catch (error) { 
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            mobile: this.filterOptions.user?.value,
        ...
        this.loading = false;
        this.tableRows = res.data.data.financialReports.data;
        this.excel_report = res.data.data.excel;
        this.totalInvestedMoney = res.data.data.totalInvestedMoney;
        this.totalProfitWithTax = res.data.data.totalProfitWithTax;
        this.total_tax = res.data.data.totalTax;
