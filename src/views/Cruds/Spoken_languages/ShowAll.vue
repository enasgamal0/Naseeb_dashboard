    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.spokenlanguages") }}</h5>
          <button
        ...
        <div
          ...
          v-if="$can('spokenlanguages create', 'spokenlanguages')"
        >
          <router-link to="/spokenlanguages/create">
            {{ $t("TITLES.addSpokenlanguages") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      >
        ...
        <template v-slot:[`item.is_active`]="{ item }">
          ...
          <div
            ...
            dir="ltr"
            style="z-index: 1"
            v-if="$can('spokenlanguages activate', 'spokenlanguages')"
          >
            <v-switch
              class="mt-2"
              color="success"
        ...
        <template v-slot:[`item.actiosn`]="{ item }">
          <div class="actions">
            <a-tooltip
              placement="bottom"
              v-if="$can('spokenlanguages delete', 'spokenlanguages')"
            >
              <template slot="title">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('spokenlanguages edit', 'spokenlanguages')"
            >
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('spokenlanguages show', 'spokenlanguages')">
              <template slot="title">
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== "1") {
        await this.$router.push({ path: "/spokenlanguages/all", query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    updateRouterQueryParam(pagenationValue) {
      ...
      // Scroll To Screen's Top After Get spokenlanguages
    ...
    async changeActivationStatus(item) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `spoken-languages/status/${item.id}`,
          data: REQUEST_DATA,
        });
        this.setTableRows();
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/spokenlanguages/edit/${item.id}` });
    },
    showItem(item) {
      this.$router.push({ path: `/spokenlanguages/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `spoken-languages/${this.itemToDelete.id}`,
        });
