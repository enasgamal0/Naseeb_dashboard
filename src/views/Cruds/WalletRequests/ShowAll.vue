    ...
    <main>
      ...
      >
        ...
        <template v-slot:[`item.actiosn`]="{ item }">
          <div
            class="actions"
            v-if="item.status == 'new' || item.status == 'جديد'"
          >
            <a-tooltip
              placement="bottom"
              v-if="$can('requestwallets edit', 'requestwallets')"
            >
              <template slot="title">
  ...
  methods: {
    ...
    async getClients() {
      ...
      try {
        let res = await this.$axios({
          method: "GET",
          url: "users?type=employee&statusRequests=accepted&page=0&limit=0",
        });
        this.loading = false;
    ...
    async changeRequestStatus(modalRequest, status) {
      ...
      if (status === "accepted" && modalRequest?.money) {
        REQUEST_DATA.append("money", modalRequest?.money);
      }

      try {
        await this.$axios({
          method: "POST",
          url: `request-wallets/change-status/${modalRequest.id}`,
          data: REQUEST_DATA,
        });
