<template>
  <div class="crud_form_wrapper">
    ...
    <div class="form_title_wrapper">
      <h4>{{ $t("TITLES.editReligioussects") }}</h4>
    </div>
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `religious-sects/${this.$route.params.id}`,
          data: REQUEST_DATA,
        });
        this.isWaitingRequest = false;
    ...
    // start show data
    async showReligioussects() {
      try {
        let res = await this.$axios({
          method: "GET",
          url: `religious-sects/${this.$route.params.id}`,
        });
        this.data.name = res.data.data.ReligiousSect.name_ar;
        this.data.nameEn = res.data.data.ReligiousSect.name_en;
        this.data.active = res.data.data.ReligiousSect.is_active;
      } catch (error) {
        this.loading = false;
