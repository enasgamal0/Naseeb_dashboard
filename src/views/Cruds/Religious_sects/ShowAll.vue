    ...
    <main>
      ...
      <div class="table_title_wrapper">
        ...
        <div
          ...
          v-if="$can('religioussects create', 'religioussects')"
        >
          <router-link to="/religioussects/create">
            {{ $t("TITLES.addReligioussects") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      >
        ...
        <template v-slot:[`item.is_active`]="{ item }">
          ...
          <div
            ...
            dir="ltr"
            style="z-index: 1"
            v-if="$can('religioussects activate', 'religioussects')"
          >
            <v-switch
              class="mt-2"
              color="success"
        ...
        <template v-slot:[`item.actiosn`]="{ item }">
          <div class="actions">
            <a-tooltip
              placement="bottom"
              v-if="$can('religioussects delete', 'religioussects')"
            >
              <template slot="title">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('religioussects edit', 'religioussects')"
            >
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('religioussects show', 'religioussects')">
              <template slot="title">
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== "1") {
        await this.$router.push({ path: "/religioussects/all", query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    updateRouterQueryParam(pagenationValue) {
      ...
      // Scroll To Screen's Top After Get religioussects
    ...
    async changeActivationStatus(item) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `religious-sects/status/${item.id}`,
          data: REQUEST_DATA,
        });
        this.setTableRows();
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/religioussects/edit/${item.id}` });
    },
    showItem(item) {
      this.$router.push({ path: `/religioussects/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `religious-sects/${this.itemToDelete.id}`,
        });
