    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.professions") }}</h5>
          <button
        ...
        <div
          ...
          v-if="$can('professions create', 'professions')"
        >
          <router-link to="/professions/create">
            {{ $t("TITLES.addProfession") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      >
        ...
        <template v-slot:[`item.is_active`]="{ item }">
          ...
          <div
            ...
            dir="ltr"
            style="z-index: 1"
            v-if="$can('professions activate', 'professions')"
          >
            <v-switch
              class="mt-2"
              color="success"
        ...
        <template v-slot:[`item.actiosn`]="{ item }">
          <div class="actions">
            <a-tooltip
              placement="bottom"
              v-if="$can('professions delete', 'professions')"
            >
              <template slot="title">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('professions edit', 'professions')"
            >
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('professions show', 'professions')">
              <template slot="title">
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== "1") {
        await this.$router.push({ path: "/professions/all", query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    updateRouterQueryParam(pagenationValue) {
      ...
      // Scroll To Screen's Top After Get professions
    ...
    async changeActivationStatus(item) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `professions/status/${item.id}`,
          data: REQUEST_DATA,
        });
        this.setTableRows();
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/professions/edit/${item.id}` });
    },
    showItem(item) {
      this.$router.push({ path: `/professions/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `professions/${this.itemToDelete.id}`,
        });
