  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `professions/${this.$route.params.id}`,
          data: REQUEST_DATA,
        });
        this.isWaitingRequest = false;
        this.$message.success(this.$t("MESSAGES.editedSuccessfully"));
        this.$router.push({ path: "/professions/all" });
      } catch (error) {
        this.isWaitingRequest = false;
    ...
    // start show data
    async showProfession() {
      try {
        let res = await this.$axios({
          method: "GET",
          url: `professions/${this.$route.params.id}`,
        });
        this.data.name = res.data.data.Profession.name_ar;
        this.data.nameEn = res.data.data.Profession.name_en;
        this.data.active = res.data.data.Profession.is_active;
      } catch (error) {
        this.loading = false;
