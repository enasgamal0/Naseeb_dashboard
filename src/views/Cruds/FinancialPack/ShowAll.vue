  ...
  methods: {
    ...
    async getUsers() {
      try {
        ...
        this.users = res.data.data?.map((ele)=>({id : ele.id , name : ele.name , value : ele.name}));
      } catch (error) {
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            package: this.filterOptions.user?.id,
        ...
        this.totalInvestedMoney = res.data.total.sub_total;
        this.totalProfitWithTax = res.data.total.tax;
        this.total_tax = res.data.total.total;
