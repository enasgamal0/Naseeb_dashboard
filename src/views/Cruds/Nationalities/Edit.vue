<template>
  <div class="crud_form_wrapper">
    ...
    <div class="form_title_wrapper">
      <h4>{{ $t("TITLES.editNationalities") }}</h4>
    </div>
...
export default {
  ...
  methods: {
    ...
    // start show data
    async showNationalities() {
      try {
        let res = await this.$axios({
          method: "GET",
          url: `nationalities/${this.$route.params.id}`,
        });
        this.data.name = res.data.data.Nationality.name_ar;
        this.data.nameEn = res.data.data.Nationality.name_en;
