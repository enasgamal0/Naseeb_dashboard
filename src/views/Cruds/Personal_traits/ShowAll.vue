    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.personaltraits") }}</h5>
          <button
        ...
        <div
          ...
          v-if="$can('personaltraits create', 'personaltraits')"
        >
          <router-link to="/personaltraits/create">
            {{ $t("TITLES.addPersonaltraits") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      >
        ...
        <template v-slot:[`item.is_active`]="{ item }">
          ...
          <div
            ...
            dir="ltr"
            style="z-index: 1"
            v-if="$can('personaltraits activate', 'personaltraits')"
          >
            <v-switch
              class="mt-2"
              color="success"
        ...
        <template v-slot:[`item.actiosn`]="{ item }">
          <div class="actions">
            <a-tooltip
              placement="bottom"
              v-if="$can('personaltraits delete', 'personaltraits')"
            >
              <template slot="title">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('personaltraits edit', 'personaltraits')"
            >
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('personaltraits show', 'personaltraits')">
              <template slot="title">
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== "1") {
        await this.$router.push({ path: "/personaltraits/all", query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    updateRouterQueryParam(pagenationValue) {
      ...
      // Scroll To Screen's Top After Get personaltraits
    ...
    async changeActivationStatus(item) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `personal-traits/status/${item.id}`,
          data: REQUEST_DATA,
        });
        this.setTableRows();
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/personaltraits/edit/${item.id}` });
    },
    showItem(item) {
      this.$router.push({ path: `/personaltraits/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `personal-traits/${this.itemToDelete.id}`,
        });
